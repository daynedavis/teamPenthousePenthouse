...
function AStar(Columns, Rows) {

	/* begin "constructor" */
	var MapColumns = Columns; // (private) map's number of columns
	var MapRows = Rows; // (private) map's number of rows
	this.GetMapColumns = GetMapColumns;
	this.GetMapRows = GetMapRows;

	var Position = new Object; // (private) current position
	this.GetPosition = GetPosition; // (public) accessor function for Position
	this.SetPosition = SetPosition; // (public) mutator function for Position
	var PreviousPosition = new Object; // (private) previous position
	this.ResetPosition = ResetPosition; // (public) reset position and previous position to start node (0,0)
	ResetPosition(); // set position to initial node upon object initialization

	this.GetPreviousPosition = GetPreviousPosition;
	this.Nodes = new Array(); // (public) 2d array, contains information for each node, Walkable should be initialized before calling Search
	GenerateNodes.call(this); // create nodes to represent map upon object initialization (this should also be called each time MapSize changes)

	var Path = []; // (private) will hold resulting path after Search has been called
	this.GetPath = GetPath; // (public) accessor function for Path
	this.Search = Search; // (public) perform search

	var Heuristic = 0; // (private) initialize with manhattan method
	this.SetHeuristic = SetHeuristic; // (public) change heuristic estimation method

	var MoveDiagonal = 0; // (private) allow diagonal movement
	this.SetDiagonal = SetDiagonal; // (public) set diagonal movement

	var TrueDiagonal = 0; // (private) when true diagonal moves will require sqrt(2) distance
	this.SetTrueDiagonal = SetTrueDiagonal; // (public) change diagonal distance requirement

	var TieBreak = 0; // (private) use a f score tie breaker in heuristic estimation
	this.SetTieBreak = SetTieBreak; // (public) change tie break setting
	/* end "constructor" */

	function GetMapColumns() { return MapColumns; }
	function GetMapRows() { return MapRows; }
	function GetPath() { return Path; }
	function GetPosition() { return Position; }
	function SetPosition(NewX, NewY) { Position.X = NewX; Position.Y = NewY; }
	function GetPreviousPosition() { return PreviousPosition; }
	function ResetPosition() {
		Position.X = 0; Position.Y = 0; // in this case start at position 0,0 (top-left)
		PreviousPosition.X = 0; PreviousPosition.Y = 0; // initialize to starting position 
	}
	function SetHeuristic(NewH) { Heuristic = NewH; }
	function SetDiagonal(Allow) { MoveDiagonal = Allow; }
	function SetTrueDiagonal(TrueD) { TrueDiagonal = TrueD; }
	function SetTieBreak(TBreak) { TieBreak = TBreak; }

	function GenerateNodes() { // (private) create appriopriate sized array of nodes
		for (x = 0; x < MapColumns; x++) {
			this.Nodes[x] = new Array();
			for (y = 0; y < MapRows; y++) {
				this.Nodes[x].push({Walkable: 0, X: x, Y: y, G: 0.0, H: 0.0, F: 0.0, Closed: 0, Visited: 0, Parent: 0});
			}
		}
	}

	function Manhattan(Start, Finish) { // (private) manhattan heuristic
		var XDist = Math.abs(Start.X - Finish.X);
		var YDist = Math.abs(Start.Y - Finish.Y);
		return XDist + YDist;
	}


	function Diagonal(Start, Finish) { // (private) diagonal heuristic
		var XDist = Math.abs(Start.X - Finish.X);
		var YDist = Math.abs(Start.Y - Finish.Y);
		if (TrueDiagonal) {
			var Diag = Math.min(XDist, YDist);
			var Straight = XDist + YDist;
			return (Math.sqrt(2) * Diag + 1 * (Straight - 2 * Diag));
		} else {
			return Math.max(XDist, YDist);
		}
	}

	function Euclidean(Start, Finish) { // (private) euclidean heurstic
		var XDistSquared = Math.pow((Start.X - Finish.X), 2);
		var YDistSquared = Math.pow((Start.Y - Finish.Y), 2);
		return Math.sqrt(XDistSquared + YDistSquared); // distance formula
	}

	function Scaling(H) { // (private) Scale H value slightly to break f ties
		var MaxPathEstimate = (MapRows * MapColumns) / 2.0; // map dimensions / 2 will work for here
		return (H * (1.0 + (1.0 / MaxPathEstimate)));
	}

	function CrossProduct(Start, Finish, H) { // (private) Cross product between start to finish and current to finish
		//this gives a preference to points along a straight line between start and finish
		var Cross = Math.abs((Start.X - Finish.X) * (Position.Y - Finish.Y) - (Position.X - Finish.X) * (Start.Y - Finish.Y));
		return (H + Cross * 0.001);
	}

	function HeuristicEstimate(Start, Finish) { // (private) estimate distance using heuristic settings
		var H = Manhattan(Start, Finish);
		if (Heuristic == 1) {
			H = Diagonal(Start, Finish);
		} else if (Heuristic == 2) {
			H = Euclidean(Start, Finish);
		}
		if (TieBreak == 1) {
			return Scaling(H);
		} else if (TieBreak == 2) {
			return CrossProduct(Start, Finish, H);
		}
		return H;
	}

	function ReCreatePath(Current) { // private function
		Path.push(Current);
	...
	function FindNeighbors(Node) { // private function
		var N = [];
		var X = Node.X; var Y = Node.Y;
		if (this.Nodes[X][Y + 1]) {
			N.push(this.Nodes[X][Y + 1]);
		}
		if (this.Nodes[X + 1] && this.Nodes[X + 1][Y]) {
			N.push(this.Nodes[X + 1][Y]);
		}
		if (this.Nodes[X][Y - 1]) {
			N.push(this.Nodes[X][Y - 1]);
		}
		...
		if (MoveDiagonal) {
			if (this.Nodes[X - 1]) {
				if (this.Nodes[X - 1][Y + 1]) {
					N.push(this.Nodes[X - 1][Y + 1]);
				}
				if (this.Nodes[X - 1][Y - 1]) {
					N.push(this.Nodes[X - 1][Y - 1]);
				}
	...
	function GetLowF(Open) { // private function
		var Lowest = Open[0];
		var Last = Open.pop();
		if (Open.length > 0) {
			Open[0] = Last;
			var Child1, Child2 = 0;
			while(1) { // until new item sinks to correct spot in heap
				Child1 = Child2;
				if (2 * Child1 + 1 < Open.length) { // if both children exist
					// check if the F cost of parent > each child
					// select the lowest of the two children
					if (Open[Child1].F >= Open[2 * Child1].F) {
						Child2 = 2 * Child1;
					}
					if (Open[Child2].F >= Open[2 * Child1 + 1].F) {
						Child2 = 2 * Child1 + 1;
					}
				} else if (2 * Child1 < Open.length) { // if only one child exists
					// check if the F cost of parent > child
					if (Open[Child1].F >= Open[2 * Child1].F) {
						Child2 = 2 * Child1;
					}
				}
				if (Child1 != Child2) { // if parent's F is > one of its children, swap them
					var Temp = Open[Child1];
	...
	function Initialize() { // (private) initialize variables for new search
		Path = [];
		for (x = 0; x < MapColumns; x++) {
			for (y = 0; y < MapRows; y++) {
				this.Nodes[x][y].G = 0.0;
				this.Nodes[x][y].H = 0.0;
				this.Nodes[x][y].F = 0.0;
				this.Nodes[x][y].Closed = 0;
				this.Nodes[x][y].Visited = 0;
				this.Nodes[x][y].Parent = 0;
			}
		}
	}
	...
	function Search(Finish) {
		...
		if (this.Nodes[Finish.X][Finish.Y].Walkable == 0) { 
			return 0; // destination is a blocked node
		}

		var Open = [];
		Open.push(this.Nodes[Position.X][Position.Y]);
		Open[0].H = HeuristicEstimate(Position, Finish);
		Open[0].F = Open[0].H;
		while (Open.length > 0) {
			var Current = GetLowF(Open);
			if (Current == this.Nodes[Finish.X][Finish.Y]) {
				PreviousPosition = Position;
				//Position = Finish; // not desirable for snake
				return ReCreatePath.call(this, Current);
			}
			Current.Closed = 1;
			var Neighbors = FindNeighbors.call(this, Current);
			for (var i = 0; i < Neighbors.length; i++) {
				...
				var GCost = Current.G + 1.0;
				if (TrueDiagonal && Neighbor.X != Current.X && Neighbor.Y != Current.Y) {
						GCost = Current.G + Math.sqrt(2);
				}
				if (!Neighbor.Visited || GCost < Neighbor.G) {
					Neighbor.G = GCost;
					if (!Neighbor.H) {
						Neighbor.H = HeuristicEstimate(Neighbor, Finish);
					}
					Neighbor.F = GCost + Neighbor.H;
					Neighbor.Parent = Current;

					// put new item in correct place in the heap by successively comparing its
					// F cost to its parents until it is in correct place or
					// at the top of the heap (if it has the lowest F cost)
					var N = 0;
					if (!Neighbor.Visited) {
						N = Open.length;
						Open.push(Neighbor);
					} else {
						N = jQuery.inArray(Neighbor, Open); // jQuery solution to indexOf and IE
					}
					Neighbor.Visited = 1;
					while (N > 0) { // while item hasn't bubbled to the top
						// check if child's f cost is < parent's f cost, if so swap them
						var ParentN = Math.floor(N / 2);
						if (Open[N].F < Open[ParentN].F) {
							var Temp = Open[ParentN];
							Open[ParentN] = Open[N];
							Open[N] = Temp;
							N = ParentN;
						} else {
        ...
        */
        ...
        me.go = function() {
            ...
            console.log(AStar(pathStart, pathEnd));
        };
